...
async def playlist(_, message):
    ...
    await message.reply_photo(
        photo=thumb,
        caption=(
            f"**{MUSIC_BOT_NAME}'s Oynatma Listesi Özelliği**\n\nOynatma Listesini Seçin, İstediğiniz **Kontrol!**"
        ),
        reply_markup=InlineKeyboardMarkup(buttons),
    )
    return
...
async def del_cmd(_, message):
    ...
    if str(count) == "all":
        ...
        return await message.reply_text(
            f"Onayla!!\nTümünü silmek istediğinden eminsin {genre} çalma listesi?",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    else:
    ...
    if not _playlist:
        await message.reply_text(
            f"Oynatma Listeniz yok {MUSIC_BOT_NAME}'s sunucu"
        )
    else:
        titlex = []
        j = 0
        ...
        for note in _playlist:
            ...
            if j == count:
                ...
                if deleted:
                    return await message.reply_text(
                        f"**Silindi {count} çalma listesindeki müzik**"
                    )
                else:
                    return await message.reply_text(
...
async def delgroupplaylist(_, message):
    ...
    if str(count) == "all":
        ...
        return await message.reply_text(
            f"Onayla!!\nGrubun tamamını silmek istediğinizden eminsiniz {genre} çalma listesi?",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
    else:
